...
def test_lock_when_expired_lock_exists(mocker):
    ...
    mocked_cleanup_lock = mocker.patch.object(DocumentLock, '_cleanup_old_lock', side_effect=original_cleanup)
    lock.lock()
    mocked_open.assert_called_with("test/path/testdoc/" + str(doc.__id__) + ".lock", "a")
...
def test_release_owned_existing_lock(mocker):
    ...
    lock.release()
    mocked_open.assert_called_with("test/path/testdoc/" + str(doc.__id__) + ".lock", "r")
    mocked_remove.assert_called_once()
